...
function uploadImage () {
	...
	if ( settings.has('cloudup.user') && settings.has('cloudup.passwd')) {
		...
		let decrypted = decipher.update(settings.get('cloudup.passwd'), 'hex', 'utf')
		...
		stream.file(image).save((err) => {
			document.getElementById('overlay').classList.toggle('hidden')
			if(err){
				showDialog('error', 'Platzipics', 'Verifique su conexión y/o credenciales.')
			}else {
				clipboard.writeText(stream.url)	
				const notify = new Notification('Platzipics', { //eslint-disable-line
					body: `Imagen cargada con éxito - ${sream.url}, el enlace se copio al portapapeles.` +
